...
signed main()
{
    ...
    auto bfs = [&](int si, int sj, int s_direction)
    {
        queue<vector<int>>q; // i, j, direction
        q.push({si, sj, s_direction});
        vis.clear();
        vis.resize(n + 1, vector<vector<int>>(m + 1, vector<int>(4, 0)));
        while(!q.empty())
        {
            auto curr = q.front();
            q.pop();
        ...
        for(int i = 0;i < n;i++)
        {
            for(int j = 0; j < m;j++)
            {
                curr_result+=(vis[i][j][0]|vis[i][j][1]|vis[i][j][2]|vis[i][j][3]);
            }
        }
        return curr_result;
    };
    ...
    for(int j = 0;j < m;j++)
    {
        result = max(result, bfs(0, j, 2));
        result = max(result, bfs(n - 1, j, 3));
    }
    for(int i = 0;i < n;i++)
    {
        result = max(result, bfs(i, 0, 0));
        result = max(result, bfs(m - 1, i, 1));
    }
